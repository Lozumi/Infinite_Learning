...
// 弗洛伊德算法，计算最短路径和前驱顶点
void Floyd(AdjMatrix A, int D[MAX][MAX], int P[MAX][MAX]);

// 创建邻接矩阵
AdjMatrix* CreateMatrix(int n);

// 打印最短路径
void PrintMinPath(AdjMatrix A, int P[MAX][MAX]);

int main() {
    int n;
    scanf("%d", &n);
    AdjMatrix* A = CreateMatrix(n);
    int D[MAX][MAX]; // 存储最短路径
    int P[MAX][MAX]; // 存储最短路径的前驱顶点
    Floyd(*A, D, P);
    PrintMinPath(*A, P);
    return 0;
}
